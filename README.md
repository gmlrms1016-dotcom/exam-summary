...
### ✨ 애니메이션 · 인터랙션 — `motion.js`
theme.js 가 자동으로 불러오니 페이지에 따로 넣지 않습니다. 색은 **화이트 = 원래 초록·주황 토큰, 다크 = 파랑·무채색**(녹색 없음)이고, 기기의 **동작 줄이기**를 켜면 움직임은 모두 빠집니다(진행 막대·맨 위로 버튼·포커스 테두리만 남음).

| 어디 | 효과 |
|------|------|
| 첫 화면 이동 | 앞으로 가면 오른쪽에서, 뒤로 가면 왼쪽에서 화면이 들어오고 카드가 차례로 떠오름 |
| 카운트다운 | 바뀐 숫자가 위에서 굴러 내려옴 · 히어로 배경 오로라 · 진행바에 빛이 지나감 |
| 상태 칩 | 시험 임박(`.pill.soon`·`.cd`)·학사일정 "진행 중"은 맥박, 시험 중(`live`)은 빨간 맥박 |
| 학사일정 | 다음 달은 오른쪽에서, 이전 달은 왼쪽에서 목록이 밀려옴 |
...
| 퀴즈 | 정답 → 보기가 톡 튀고 색종이 · 오답 → 좌우로 흔들림 · 점수판·틀린 문제 버튼 숫자가 바뀌면 톡 |
| 공통 | 카드에 마우스를 따라다니는 빛(PC) · 버튼 누르면 물결 · 눌림 · 아이콘 스프링 · 잠긴 버튼은 누르면 살짝 흔들림 |

- ⚠️ **카운트다운 카드(`.hero-clock`)는 숨은 이동 자리**라 마우스·터치에 반응하는 효과(빛·물결·눌림·흔들림)를 절대 넣지 않습니다. 숫자 굴림은 시간이 바뀔 때만 일어납니다.
