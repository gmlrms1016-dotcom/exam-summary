...
| 어디 | 효과 |
|------|------|
| 누르는 느낌 | 버튼은 `scale(.94)`, 카드는 `scale(.975)` 로 쏙 들어갔다가 스프링으로 튕겨 나옴 · 모바일 회색 탭 하이라이트 제거 |
| 첫 화면 이동 | 새 화면은 들어오고 **이전 화면은 반대 방향으로 밀려 나감**(`.mo-leaving`) · 카드는 차례로 떠오름 |
| 카운트다운 | **자리별 숫자 굴림**(바뀐 자리만 `.mo-d.mo-tick`) · 히어로 배경 오로라 · 진행바에 빛이 지나감 |
| 상태 칩 | 시험 임박(`.pill.soon`·`.cd`)·학사일정 "진행 중"은 맥박, 시험 중(`live`)은 빨간 맥박 |
| 학사일정 | 불러오는 동안 **스켈레톤**(`.mo-sk`) · 다음 달은 오른쪽, 이전 달은 왼쪽에서 밀려옴 |
| 과목·주차 페이지 | 스크롤하면 카드·문제가 차례로 떠오름 · 머리말에 빛이 지나감 · 제목이 단어별로 등장 · 목차·주차 버튼이 톡톡 나옴 |
| 상단 바 | 스크롤해서 머리말이 지나가면 위에서 **반투명 상단 바(`#mo-bar`)** 가 내려옴 — 뒤로 버튼 + 과목명(누르면 맨 위로) |
| 읽기 도우미 | 맨 위 진행 막대 + 오른쪽 아래 **맨 위로** 버튼(진행 원, '틀린 문제 복사' 버튼 위) · 목차 누르면 부드럽게 이동 후 도착한 카드 테두리가 반짝 |
| 퀴즈 | 정답 → 보기가 톡 튀고 색종이 + **짧은 진동(12ms)** · 오답 → 좌우로 흔들림 · 점수판·틀린 문제 버튼 숫자가 바뀌면 톡 · 입력칸 포커스 링 |
| 틀린 문제 모음 | 폰에서는 **아래에서 올라오는 시트**(손잡이를 아래로 끌면 닫힘), PC 에서는 가운데 카드 — `review.js` |
| 알림 | 복사처럼 결과만 알리면 되는 일은 아래에서 올라오는 **토스트**(`window.moToast("문구")`) |
| 공통 | 카드에 마우스를 따라다니는 빛(PC) · 아이콘 스프링 · 잠긴 버튼은 누르면 살짝 흔들림 |

- **진동**: 정답일 때만 아주 짧게. 안드로이드 크롬 등은 `navigator.vibrate`, 아이폰 사파리는 진동 API 가 없어 iOS 17.4+ 의 스위치 토글 햅틱(숨겨 둔 `<input type="checkbox" switch>`)을 빌려 씁니다. 둘 다 없으면 조용히 넘어갑니다.
- ⚠️ **카운트다운 카드(`.hero-clock`)는 숨은 이동 자리**라 마우스·터치에 반응하는 효과(빛·눌림·흔들림)를 절대 넣지 않습니다. 숫자 굴림은 시간이 바뀔 때만 일어납니다.
...
7. **퀴즈 추가 ★ 굳이 말하지 않아도 반드시**
   ...
   - **언어 과목 정리는 설명보다 예제 소스코드 중심** — 배운 범위 안에서 예제를 많이 넣고 설명은 주석으로, 모든 예제는 직접 컴파일·실행한 **실제 출력 결과**(`pre.io` / 주차 원고는 ```output) 를 붙입니다. 사이트 · 노션 · 주차 페이지에 같은 내용으로.
